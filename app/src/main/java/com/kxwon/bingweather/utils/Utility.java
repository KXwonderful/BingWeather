...
public class Utility {
    ...
    /**
     * 将返回的 JSON 数据解析成 SearchCity 实体类
     */
    public static SearchCity handleCityResponse(String response){
        try {
            JSONObject jsonObject = new JSONObject(response);
            //JSONArray jsonArray = jsonObject.getJSONArray("HeWeather5");
            //String cityContent = jsonArray.getJSONObject(0).toString();
            String cityContent = jsonObject.toString();
            return new Gson().fromJson(cityContent,SearchCity.class);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return null;
